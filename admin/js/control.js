...
const SAMPLE_QUESTIONS = [
    ...
    { order: 11, difficulty: "medium", questionText: "Which function in NumPy computes the dot product of two arrays?", options: ["numpy.multiply()", "numpy.dot()", "numpy.sum()", "numpy.cross()"], correctAnswer: "B", basePoints: 100 },
    { order: 12, difficulty: "medium", questionText: "Which Scikit-learn function splits data into training and testing sets?", options: ["split_data()", "train_test()", "train_test_split()", "data_split()"], correctAnswer: "C", basePoints: 100 },
    { order: 13, difficulty: "medium", questionText: "Which metric function evaluates how far predictions deviate from actual values?", options: ["accuracy_score()", "recall_score()", "mean_squared_error()", "f1_score()"], correctAnswer: "C", basePoints: 100 },
    { order: 14, difficulty: "medium", questionText: "Which activation function outputs values between 0 and 1?", options: ["ReLU", "Softmax", "Tanh", "Sigmoid"], correctAnswer: "D", basePoints: 100 },
    { order: 15, difficulty: "medium", questionText: "Which loss function is commonly used for binary classification?", options: ["mean_squared_error", "categorical_crossentropy", "binary_crossentropy", "hinge_loss"], correctAnswer: "C", basePoints: 100 },
    { order: 16, difficulty: "medium", questionText: "Which algorithm uses a heuristic function to find the optimal path?", options: ["Breadth First Search", "Depth First Search", "A* Search", "Minimax"], correctAnswer: "C", basePoints: 100 },
    // 🔴 HARD — Code Debugging & Fix-the-Bug (200 pts)
    { order: 17, difficulty: "hard", questionText: "X = np.array([1, 2, 3])\nw = np.array([[0.2], [0.5], [0.1]])\ny = X @ w\n\nWhat will fix the shape issue to make output (1,1)?", options: ["X = np.array([[1, 2, 3]])", "w = np.array([0.2, 0.5, 0.1])", "y = np.dot(w, X)", "X = X.reshape(3, 1)"], correctAnswer: "A", basePoints: 100 },
    { order: 18, difficulty: "hard", questionText: "X = [[1],[2],[3],[4]]\ny = [0,1,0]\nX_train, X_test, y_train, y_test = train_test_split(X, y)\n\nWhich option fixes the error?", options: ["y = [0,1,0,1]", "X = [[1],[2],[3]]", "X = [1,2,3]", "train_test_split(y, X)"], correctAnswer: "A", basePoints: 100 },
    { order: 19, difficulty: "hard", questionText: "a = [[1,2],[3,4]]\nb = a.copy()\nb[0][0] = 99\nprint(a)\n\nHow do you prevent modifying `a`?", options: ["import copy; b = copy.deepcopy(a)", "b = list(a)", "b = a[:]", "b = tuple(a)"], correctAnswer: "A", basePoints: 100 },
    { order: 20, difficulty: "hard", questionText: "x = [1,2,3]\ny = [1,2,3]\nif x is y:\n    print(\"Equal\")\n\nWhich fix correctly compares values?", options: ["if x == y:", "if id(x) == id(y):", "if x.equals(y):", "if list(x) is list(y):"], correctAnswer: "A", basePoints: 100 },
    { order: 21, difficulty: "hard", questionText: "def update(d, key, value):\n    temp = d\n    temp[key] = value\n    return temp\n\ndata = {\"a\":1}\nnew_data = update(data, \"b\", 2)\n\nWhat is the correct fix to avoid mutating `data`?", options: ["temp = d.copy()", "temp = dict(d)", "import copy; temp = copy.deepcopy(d)", "All of the above"], correctAnswer: "D", basePoints: 100 }
];
